...
resource "aws_security_group" "rds_access" {
 ...
 description = "RDS Access on Port 3306"
...
resource "aws_db_instance" "dbinstance2" {
  ...
  engine               = "mysql"
  engine_version       = "8.0.44"          ### var.engine_version[11] use for postgresql
  instance_class       = var.instance_class
  db_name              = var.rds_db_name
  username             = "admin"
  password             = var.password
  parameter_group_name = "default.mysql8.0"           ### var.parameter_group_name[1] use for postgresql
...
###  availability_zone = var.availability_zone[0]  ## NOT ENABLE FOR MULTI-AZ OPTION
  vpc_security_group_ids = [aws_security_group.rds_access.id]        ###var.vpc_security_group_ids
